 ...
 * Sends push via FCM when a friend request is sent or accepted.
 */
...
serve(async (req) => {
  ...
  try {
    ...
    const sent = await sendToUser(supabase, accessToken, FIREBASE_PROJECT_ID, to_user_id, heading, content);
    console.log(`[friend-request] Sent ${type} to ${to_user_id.substring(0, 8)}... (${sent} devices)`);
