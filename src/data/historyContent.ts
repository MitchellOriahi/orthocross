export type MiniIslandType = "story" | "glossary" | "timeline" | "people" | "map" | "quick_quiz" | "review" | "reward_chest";

export interface MiniIsland {
  id: string;
  type: MiniIslandType;
  title: string;
  estSeconds?: number;
  ...
  locations?: Array<{ name: string; coords: { x: number; y: number } }>;
  questions?: Array<{
    q: string;
...
export interface Segment {
  segmentId: string;
  leadsToBigIslandId: string;
  unlockCondition: string;
  miniIslands: MiniIsland[];
  visual: {
    curve: "s";
    nodeSpacingPx: number;
    zigzagOffsetPx: number;
    miniNodeDiameterPx: number;
  };
}
  ...
  campaigns: [
    {
      ...
      segments: [
        {
          segmentId: "eo_1_segment",
          leadsToBigIslandId: "eo_1",
          ...
          miniIslands: [
            {
              ...
              estSeconds: 90,
              content: "The Orthodox Church began on the Day of Pentecost. The Holy Spirit descended on the Apostles like fire, transforming them from fearful followers into bold leaders who spoke in different languages. Peter preached and 3,000 people joined the Church that day.",
              rewards: { xp: 10, coins: 5, armorShards: 1 },
            ...
            },
            {
              ...
              title: "Key Terms",
              items: [
                { term: "Apostolic Succession", def: "The unbroken line of bishops from the Apostles to today" },
                { term: "Martyrs", def: "Those who died bravely for their faith in Christ" },
                { term: "Apostolic Fathers", def: "Church leaders who knew the Apostles or their disciples" }
              ],
              rewards: { xp: 10, coins: 5, armorShards: 1 },
            ...
            },
            {
              ...
              title: "Early Martyrs",
              people: [
                { name: "Ignatius of Antioch", description: "Thrown to lions in Rome" },
                { name: "Polycarp of Smyrna", description: "86 years old when martyred by fire" },
                { name: "Perpetua", description: "Young mother martyred in Carthage" }
              ],
              rewards: { xp: 10, coins: 5, armorShards: 1 },
              state: "locked" as const,
              icon: "users"
            },
            {
              ...
              questions: [
                {
                  q: "What event marks the birth of the Church?",
                  options: [
                    { text: "Pentecost", isCorrect: true },
                    { text: "Council of Nicaea", isCorrect: false },
                    { text: "Edict of Milan", isCorrect: false }
                  ]
                },
                {
                  q: "What is Apostolic Succession?",
                  options: [
                    { text: "Line of bishops from the Apostles", isCorrect: true },
                    { text: "A rotation of councils", isCorrect: false },
                    { text: "Election of emperors", isCorrect: false }
                  ]
                }
              ],
              rewards: { xp: 15, coins: 8, armorShards: 1 },
              state: "locked" as const,
              icon: "check"
            },
            {
              id: "eo_1_chest",
              type: "reward_chest" as const,
              title: "Mid-Path Chest",
              rewards: { xp: 0, coins: 20, armorShards: 1, consumables: ["heart_refill"] },
              state: "locked" as const,
              icon: "chest"
            },
            {
              ...
              title: "Speed Review",
              poolRef: ["eo_1_m1", "eo_1_m2", "eo_1_m3", "eo_1_m4"],
              rewards: { xp: 15, coins: 6, armorShards: 1 },
              state: "locked" as const,
              icon: "lightning"
            }
          ],
